...
namespace Bitcoin_Wallet
{
    class BitcoinWallet
    {
        ...
        {
            //  Load the wallet and initialize variables
            ...
            //  Derive the first address to be used as change address
            ...
            //  Loop the first 10 accounts
            for (int i = 0; i < 10; i++)
            {
                //  Continuously derive addresses
                ...
                //  Use getBalance to retrieve outpoints and transaction history
        ...
        private static void ShowHistory(string password, string walletName)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                //  Use GetBalance to retrieve transaction activity of received coins
                var transactions = client.GetBalance(BitcoinAddress.Create(address, currentNetwork), false).Result;

                //  Display the receive history of the wallet
                foreach (var entry in transactions.Operations)
                {
                    foreach (var coin in entry.ReceivedCoins)
                    {
                        WriteLine($"Transaction ID: {coin.Outpoint}; Recieved coins: {amount.ToDecimal(MoneyUnit.BTC)}");
                    }
                }
            }
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                //  Use GetBalance to retrieve transaction activity of spent coins
                var transactions = client.GetBalance(BitcoinAddress.Create(address, currentNetwork), false).Result;

                //  Display the spent history of the wallet
                foreach (var entry in transactions.Operations)
                {
        ...
        private static void ShowBalance(string password, string walletName)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                //  Use getBalance to retrieve outpoints and wallet history
                var balance = client.GetBalance(BitcoinAddress.Create(address, currentNetwork), true).Result;
                
                //  Display the balances of each address while summing them up to display the total balance
                foreach (var entry in balance.Operations)
                {
                    foreach (var coin in entry.ReceivedCoins)
                    {
        ...
        private static void Receive(string password, string walletName)
        {
            try
            {
                ...
                //  Derive continuously and display the wallet address
                for (int i = 0; i < 10; i++)
                {
                    KeyPath keyPath = new KeyPath(testNetBTCDerivationPath + i);
                    ExtKey key = masterKey.Derive(keyPath);
        ...
        private static void RecoverWallet(string password, string rawMnemonic, string walletName)
        {
            try
            {
                //  Process the mnemonic string, encrypt it, and save to disk
        ...
        private static void CreateWallet()
        {
            ...
            while (failure)
            {
                try
                {
                    ...
                    //  Derive the addresses and print the address -> private key of each derivation
                    for (int i = 0; i < 10; i++)
                    {
                        KeyPath keyPath = new KeyPath(testNetBTCDerivationPath + i);
                        ExtKey key = masterKey.Derive(keyPath);
                        WriteLine($"Address: {key.PrivateKey.PubKey.GetAddress(ScriptPubKeyType.Legacy, currentNetwork)} -> Privatekey: {key.PrivateKey.GetBitcoinSecret(currentNetwork)} ");
                    }
